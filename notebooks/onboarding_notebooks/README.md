...
- `03_downstream_training_demo.ipynb`: trains a small CPU-only scikit-learn MLP on the included features. The official repository training path is `code/downstream/train_single_correctness_head.py`.
- `04_pretrained_inference_demo.ipynb`: inspects pretrained correctness-head prediction files and queries candidate-level predictions.
